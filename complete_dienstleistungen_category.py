#!/usr/bin/env python3
"""
Complete Dienstleistungen category
"""

import os

subcategories = [
    ('DienstleistungenAltenpflegePage.js', 'Altenpflege'),
    ('DienstleistungenAutoRadBootPage.js', 'Auto, Rad & Boot'),
    ('DienstleistungenBabysitterPage.js', 'Babysitter/-in & Kinderbetreuung'),
    ('DienstleistungenElektronikPage.js', 'Elektronik'),
    ('DienstleistungenHausGartenPage.js', 'Haus & Garten'),
    ('DienstleistungenKuenstlerMusikerPage.js', 'Künstler/-in & Musiker/-in'),
    ('DienstleistungenReiseEventPage.js', 'Reise & Event'),
    ('DienstleistungenTierbetreuungPage.js', 'Tierbetreuung & Training'),
    ('DienstleistungenUmzugTransportPage.js', 'Umzug & Transport'),
    ('DienstleistungenWeiterePage.js', 'Weitere Dienstleistungen'),
]

base_path = '/Volumes/Kerem Aydin/Projeler/Kleinanzegen/24.11.2025/app/frontend/src'

print("📋 Dienstleistungen Category Update")
print("=" * 60)

# Step 1: Add import and route
print("\n1️⃣  Adding DienstleistungenPage import and route...")
app_js_path = os.path.join(base_path, 'App.js')

with open(app_js_path, 'r', encoding='utf-8') as f:
    app_content = f.read()

if 'import DienstleistungenPage' not in app_content:
    app_content = app_content.replace(
        "import WeitereEintrittskartenTicketsPage from './WeitereEintrittskartenTicketsPage';",
        "import WeitereEintrittskartenTicketsPage from './WeitereEintrittskartenTicketsPage';\nimport DienstleistungenPage from './DienstleistungenPage';"
    )
    
    app_content = app_content.replace(
        '          <Route path="/Dienstleistungen/Altenpflege"',
...
for filename, subcategory_name in subcategories:
    ...
    if 'Dienstleistungen' in content and 'Kategorien</h3>' in content and subcategory_name in content:
...
main_page_path = os.path.join(base_path, 'DienstleistungenPage.js')
...
const DienstleistungenPage = () => {
    ...
    const subcategories = [
        { name: 'Alle', count: 521 },
        { name: 'Altenpflege', count: 87, route: '/Dienstleistungen/Altenpflege' },
        { name: 'Auto, Rad & Boot', count: 0, route: '/Dienstleistungen/Auto-Rad-Boot' },
        { name: 'Babysitter/-in & Kinderbetreuung', count: 0, route: '/Dienstleistungen/Babysitter-Kinderbetreuung' },
        { name: 'Elektronik', count: 0, route: '/Dienstleistungen/Elektronik' },
        { name: 'Haus & Garten', count: 0, route: '/Dienstleistungen/Haus-Garten' },
        { name: 'Künstler/-in & Musiker/-in', count: 45, route: '/Dienstleistungen/Künstler-Musiker' },
        { name: 'Reise & Event', count: 0, route: '/Dienstleistungen/Reise-Event' },
        { name: 'Tierbetreuung & Training', count: 0, route: '/Dienstleistungen/Tierbetreuung-Training' },
        { name: 'Umzug & Transport', count: 124, route: '/Dienstleistungen/Umzug-Transport' },
        { name: 'Weitere Dienstleistungen', count: 265, route: '/Dienstleistungen/Weitere-Dienstleistungen' }
    ];
        ...
        const fetchListings = async () => {
            try {
                ...
                if (response.ok) {
                    ...
                    setListings(data.filter(listing => listing.category === 'Dienstleistungen'));
                }
            } catch (error) {
            ...
            <div className="max-w-[1400px] mx-auto px-4 py-6">
                <div className="flex gap-6">
                    <aside className="w-96 flex-shrink-0 bg-white rounded-2xl shadow-lg p-6 h-fit sticky top-6">
                        ...
                        <div className="space-y-1">
                            {subcategories.map((sub) => (
                                <button key={sub.name} onClick={() => navigate(sub.route || '/Dienstleistungen')} className="w-full text-left px-4 py-3 rounded-lg transition-all flex items-center justify-between hover:bg-gray-50 text-gray-700 group">
                                    <span className="text-sm group-hover:text-cyan-600 transition-colors">{sub.name}</span>
                                    <div className="flex items-center gap-2">
                                        <span className="text-xs text-gray-400">({sub.count.toLocaleString('de-DE')})</span>
                                        <svg className="w-4 h-4 text-gray-400 group-hover:text-cyan-600 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
                                    </div>
                                </button>
                            ))}
                        </div>
                    ...
                    <div className="flex-1">
                        <div className="bg-gradient-to-r from-cyan-500 to-blue-600 rounded-2xl shadow-xl p-8 mb-6 relative overflow-hidden">
                            ...
                            <div className="relative z-10">
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-4">
                                        <div className="w-16 h-16 bg-white bg-opacity-20 backdrop-blur-sm rounded-2xl flex items-center justify-center"><span className="text-5xl">🛠️</span></div>
                                        <div><h1 className="text-4xl font-bold text-white mb-1">Dienstleistungen</h1><p className="text-white text-lg opacity-90">Alle Kategorien</p></div>
                                    </div>
                        ...
                        <div className="bg-white rounded-lg shadow-sm p-6">
                            ...
                            {loading ? <div className="flex justify-center items-center py-12"><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-cyan-600"></div></div> : listings.length === 0 ? <div className="text-center py-12"><p className="text-gray-500 text-lg">Keine Anzeigen gefunden</p></div> : (
                                    ...
                                    {listings.map((listing) => (
                                        <div key={listing.id} className="bg-white rounded-xl shadow-md hover:shadow-xl transition-all duration-300 cursor-pointer group overflow-hidden flex" onClick={() => navigate(`/listing/${listing.id}`)}>
                                            ...
                                            <div className="flex-1 p-6 flex flex-col justify-between">
                                                <div><h3 className="font-semibold text-xl text-gray-900 mb-2 group-hover:text-cyan-600 transition-colors">{listing.title}</h3><p className="text-gray-600 text-sm line-clamp-2 mb-3">{listing.description || 'Keine Beschreibung verfügbar'}</p></div>
                                                <div className="flex items-center justify-between">
                                                    <p className="text-2xl font-bold text-cyan-600">{listing.price ? `${listing.price} €` : 'VB'}</p>
...
'''
    ...
    print("   ✅ Created DienstleistungenPage.js")
else:
    print("   ℹ️  Main page already exists")

print("\n🎉 Dienstleistungen category complete!")
