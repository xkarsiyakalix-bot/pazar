#!/usr/bin/env python3
"""
Complete Nachbarschaftshilfe category
"""

import os

subcategories = [
    ('NachbarschaftshilfePage.js', 'Nachbarschaftshilfe'),
]

base_path = '/Volumes/Kerem Aydin/Projeler/Kleinanzegen/24.11.2025/app/frontend/src'

print("📋 Nachbarschaftshilfe Category Update")
print("=" * 60)

# Step 1: Add import and route
print("\n1️⃣  Adding NachbarschaftshilfeMainPage import and route...")
app_js_path = os.path.join(base_path, 'App.js')

with open(app_js_path, 'r', encoding='utf-8') as f:
    app_content = f.read()

if 'import NachbarschaftshilfeMainPage' not in app_content:
    app_content = app_content.replace(
        "import WeitereUnterrichtKursePage from './WeitereUnterrichtKursePage';",
        "import WeitereUnterrichtKursePage from './WeitereUnterrichtKursePage';\nimport NachbarschaftshilfeMainPage from './NachbarschaftshilfeMainPage';"
    )
    
    app_content = app_content.replace(
        '          <Route path="/Nachbarschaftshilfe/Nachbarschaftshilfe"',
        '          <Route path="/Nachbarschaftshilfe" element={<NachbarschaftshilfeMainPage />} />\n          <Route path="/Nachbarschaftshilfe/Nachbarschaftshilfe"'
    )
...
# Step 2: Update subcategory page
print("\n2️⃣  Updating subcategory page...")
...
nav_template = '''                <div className="mb-6 pb-6 border-b border-gray-200">
                    ...
                    <div className="space-y-2 mt-3">
                        ...
                        <button className="text-left px-3 py-2 rounded-lg text-sm transition-all bg-red-600 text-white shadow-md flex items-center justify-between ml-8" style={{{{ width: 'calc(100% - 2rem)' }}}}>
                            <span>Nachbarschaftshilfe</span>
                            <button onClick={{(e) => {{ e.stopPropagation(); navigate('/Nachbarschaftshilfe'); }}}} className="text-white hover:text-red-200 transition-colors" title="Kategorie schließen">
...
for filename, subcategory_name in subcategories:
    ...
    if 'Nachbarschaftshilfe' in content and 'Kategorien</h3>' in content and content.count('Nachbarschaftshilfe') > 3:
...
const NachbarschaftshilfeMainPage = () => {
    ...
    const subcategories = [
        { name: 'Alle', count: 3 },
        { name: 'Nachbarschaftshilfe', count: 3, route: '/Nachbarschaftshilfe/Nachbarschaftshilfe' }
    ];
        ...
        const fetchListings = async () => {
            try {
                ...
                if (response.ok) {
                    ...
                    setListings(data.filter(listing => listing.category === 'Nachbarschaftshilfe'));
                }
            } catch (error) {
            ...
            <div className="max-w-[1400px] mx-auto px-4 py-6">
                <div className="flex gap-6">
                    <aside className="w-96 flex-shrink-0 bg-white rounded-2xl shadow-lg p-6 h-fit sticky top-6">
                        ...
                        <div className="space-y-1">
                            {subcategories.map((sub) => (
                                <button key={sub.name} onClick={() => navigate(sub.route || '/Nachbarschaftshilfe')} className="w-full text-left px-4 py-3 rounded-lg transition-all flex items-center justify-between hover:bg-gray-50 text-gray-700 group">
                                    <span className="text-sm group-hover:text-pink-600 transition-colors">{sub.name}</span>
                                    <div className="flex items-center gap-2">
                                        <span className="text-xs text-gray-400">({sub.count.toLocaleString('de-DE')})</span>
                                        <svg className="w-4 h-4 text-gray-400 group-hover:text-pink-600 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
                                    </div>
                                </button>
                            ))}
                        </div>
                    ...
                    <div className="flex-1">
                        <div className="bg-gradient-to-r from-pink-500 to-rose-600 rounded-2xl shadow-xl p-8 mb-6 relative overflow-hidden">
                            ...
                            <div className="relative z-10">
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div><h1 className="text-4xl font-bold text-white mb-1">Nachbarschaftshilfe</h1><p className="text-white text-lg opacity-90">Alle Kategorien</p></div>
                                    </div>
                        ...
                        <div className="bg-white rounded-lg shadow-sm p-6">
                            ...
                            {loading ? <div className="flex justify-center items-center py-12"><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-pink-600"></div></div> : listings.length === 0 ? <div className="text-center py-12"><p className="text-gray-500 text-lg">Keine Anzeigen gefunden</p></div> : (
                                    ...
                                    {listings.map((listing) => (
                                        <div key={listing.id} className="bg-white rounded-xl shadow-md hover:shadow-xl transition-all duration-300 cursor-pointer group overflow-hidden flex" onClick={() => navigate(`/listing/${listing.id}`)}>
                                            ...
                                            <div className="flex-1 p-6 flex flex-col justify-between">
                                                <div><h3 className="font-semibold text-xl text-gray-900 mb-2 group-hover:text-pink-600 transition-colors">{listing.title}</h3><p className="text-gray-600 text-sm line-clamp-2 mb-3">{listing.description || 'Keine Beschreibung verfügbar'}</p></div>
                                                <div className="flex items-center justify-between">
                                                    <p className="text-2xl font-bold text-pink-600">{listing.price ? `${listing.price} €` : 'VB'}</p>
...
'''
    ...
    print("   ✅ Created NachbarschaftshilfeMainPage.js")
else:
    print("   ℹ️  Main page already exists")

print("\n🎉 Nachbarschaftshilfe category complete!")
print("\n" + "=" * 60)
