#!/usr/bin/env python3
"""
Complete Verschenken & Tauschen category
"""

import os

subcategories = [
    ('TauschenPage.js', 'Tauschen'),
    ('VerleihenPage.js', 'Verleihen'),
    ('VerschenkenPage.js', 'Verschenken'),
]

base_path = '/Volumes/Kerem Aydin/Projeler/Kleinanzegen/24.11.2025/app/frontend/src'

print("📋 Verschenken & Tauschen Category Update")
print("=" * 60)

# Step 1: Add import and route
print("\n1️⃣  Adding VerschenkenTauschenPage import and route...")
app_js_path = os.path.join(base_path, 'App.js')

with open(app_js_path, 'r', encoding='utf-8') as f:
    app_content = f.read()

if 'import VerschenkenTauschenPage' not in app_content:
    app_content = app_content.replace(
        "import DienstleistungenWeiterePage from './DienstleistungenWeiterePage';",
        "import DienstleistungenWeiterePage from './DienstleistungenWeiterePage';\nimport VerschenkenTauschenPage from './VerschenkenTauschenPage';"
    )
    
    app_content = app_content.replace(
        '          <Route path="/Verschenken-Tauschen/Tauschen"',
        '          <Route path="/Verschenken-Tauschen" element={<VerschenkenTauschenPage />} />\n          <Route path="/Verschenken-Tauschen/Tauschen"'
    )
...
nav_template = '''                <div className="mb-6 pb-6 border-b border-gray-200">
                    ...
                    <div className="space-y-2 mt-3">
                        ...
                        <button className="text-left px-3 py-2 rounded-lg text-sm transition-all bg-red-600 text-white shadow-md flex items-center justify-between ml-8" style={{{{ width: 'calc(100% - 2rem)' }}}}>
                            ...
                            <button onClick={{(e) => {{ e.stopPropagation(); navigate('/Verschenken-Tauschen'); }}}} className="text-white hover:text-red-200 transition-colors" title="Kategorie schließen">
...
for filename, subcategory_name in subcategories:
    ...
    if 'Verschenken & Tauschen' in content and 'Kategorien</h3>' in content:
...
const VerschenkenTauschenPage = () => {
    ...
    const subcategories = [
        { name: 'Alle', count: 196 },
        { name: 'Tauschen', count: 34, route: '/Verschenken-Tauschen/Tauschen' },
        { name: 'Verleihen', count: 3, route: '/Verschenken-Tauschen/Verleihen' },
        { name: 'Verschenken', count: 159, route: '/Verschenken-Tauschen/Verschenken' }
    ];
        ...
        const fetchListings = async () => {
            try {
                ...
                if (response.ok) {
                    ...
                    setListings(data.filter(listing => listing.category === 'Verschenken & Tauschen'));
                }
            } catch (error) {
            ...
            <div className="max-w-[1400px] mx-auto px-4 py-6">
                <div className="flex gap-6">
                    <aside className="w-96 flex-shrink-0 bg-white rounded-2xl shadow-lg p-6 h-fit sticky top-6">
                        ...
                        <div className="space-y-1">
                            {subcategories.map((sub) => (
                                <button key={sub.name} onClick={() => navigate(sub.route || '/Verschenken-Tauschen')} className="w-full text-left px-4 py-3 rounded-lg transition-all flex items-center justify-between hover:bg-gray-50 text-gray-700 group">
                                    <span className="text-sm group-hover:text-emerald-600 transition-colors">{sub.name}</span>
                    ...
                    <div className="flex-1">
                        <div className="bg-gradient-to-r from-emerald-500 to-green-600 rounded-2xl shadow-xl p-8 mb-6 relative overflow-hidden">
                            ...
                            <div className="relative z-10">
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div><h1 className="text-4xl font-bold text-white mb-1">Verschenken & Tauschen</h1><p className="text-white text-lg opacity-90">Alle Kategorien</p></div>
                                    </div>
                        ...
                        <div className="bg-white rounded-lg shadow-sm p-6">
                            ...
                            {loading ? <div className="flex justify-center items-center py-12"><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-emerald-600"></div></div> : listings.length === 0 ? <div className="text-center py-12"><p className="text-gray-500 text-lg">Keine Anzeigen gefunden</p></div> : (
                                    ...
                                    {listings.map((listing) => (
                                        <div key={listing.id} className="bg-white rounded-xl shadow-md hover:shadow-xl transition-all duration-300 cursor-pointer group overflow-hidden flex" onClick={() => navigate(`/listing/${listing.id}`)}>
                                            ...
                                            <div className="flex-1 p-6 flex flex-col justify-between">
                                                <div><h3 className="font-semibold text-xl text-gray-900 mb-2 group-hover:text-emerald-600 transition-colors">{listing.title}</h3><p className="text-gray-600 text-sm line-clamp-2 mb-3">{listing.description || 'Keine Beschreibung verfügbar'}</p></div>
                                                <div className="flex items-center justify-between">
                                                    <p className="text-2xl font-bold text-emerald-600">{listing.price ? `${listing.price} €` : 'Zu verschenken'}</p>
...
'''
    ...
    print("   ✅ Created VerschenkenTauschenPage.js")
else:
    print("   ℹ️  Main page already exists")

print("\n🎉 Verschenken & Tauschen category complete!")
