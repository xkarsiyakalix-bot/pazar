#!/usr/bin/env python3
"""
Complete Musik, Filme & Bücher category
"""

import os

subcategories = [
    ('BuecherZeitschriftenPage.js', 'Bücher & Zeitschriften'),
    ('BueroSchreibwarenPage.js', 'Büro & Schreibwaren'),
    ('ComicsPage.js', 'Comics'),
    ('FachbuecherSchuleStudiumPage.js', 'Fachbücher, Schule & Studium'),
    ('FilmDVDPage.js', 'Film & DVD'),
    ('MusikCDsPage.js', 'Musik & CDs'),
    ('MusikinstrumentePage.js', 'Musikinstrumente'),
    ('WeitereMusikFilmeBuecherPage.js', 'Weitere Musik, Filme & Bücher'),
]

base_path = '/Volumes/Kerem Aydin/Projeler/Kleinanzegen/24.11.2025/app/frontend/src'

print("📋 Musik, Filme & Bücher Category Update")
print("=" * 60)

# Step 1: Add import and route to App.js
print("\n1️⃣  Adding MusikFilmeBuecherPage import and route...")
app_js_path = os.path.join(base_path, 'App.js')

with open(app_js_path, 'r', encoding='utf-8') as f:
    app_content = f.read()

if 'import MusikFilmeBuecherPage' not in app_content:
    app_content = app_content.replace(
        "import WeiteresFreizeitHobbyNachbarschaftPage from './WeiteresFreizeitHobbyNachbarschaftPage';",
        "import WeiteresFreizeitHobbyNachbarschaftPage from './WeiteresFreizeitHobbyNachbarschaftPage';\nimport MusikFilmeBuecherPage from './MusikFilmeBuecherPage';"
    )
    
    app_content = app_content.replace(
        '          <Route path="/Musik-Filme-Bücher/Bücher-Zeitschriften"',
        '          <Route path="/Musik-Filme-Bücher" element={<MusikFilmeBuecherPage />} />\n          <Route path="/Musik-Filme-Bücher/Bücher-Zeitschriften"'
    )
...
nav_template = '''                <div className="mb-6 pb-6 border-b border-gray-200">
                    ...
                    <div className="space-y-2 mt-3">
                        <button onClick={{() => navigate('/Musik-Filme-Bücher')}} className="text-left px-3 py-2 rounded-lg text-sm transition-all bg-gray-100 text-gray-700 hover:bg-gray-200 flex items-center justify-between group ml-4" style={{{{ width: 'calc(100% - 1rem)' }}}}>
                        ...
                        <button className="text-left px-3 py-2 rounded-lg text-sm transition-all bg-red-600 text-white shadow-md flex items-center justify-between ml-8" style={{{{ width: 'calc(100% - 2rem)' }}}}>
                            ...
                            <button onClick={{(e) => {{ e.stopPropagation(); navigate('/Musik-Filme-Bücher'); }}}} className="text-white hover:text-red-200 transition-colors" title="Kategorie schließen">
...
for filename, subcategory_name in subcategories:
    ...
    if 'Musik, Filme & Bücher' in content and 'Kategorien</h3>' in content:
...
main_page_path = os.path.join(base_path, 'MusikFilmeBuecherPage.js')

if not os.path.exists(main_page_path):
    main_page_content = '''import React, { useState, useEffect } from 'react';
import { useNavigate } from 'react-router-dom';

const MusikFilmeBuecherPage = () => {
    const navigate = useNavigate();
    const [listings, setListings] = useState([]);
    const [loading, setLoading] = useState(true);

    const subcategories = [
        { name: 'Alle', count: 3370 },
        { name: 'Bücher & Zeitschriften', count: 2091, route: '/Musik-Filme-Bücher/Bücher-Zeitschriften' },
        { name: 'Büro & Schreibwaren', count: 33, route: '/Musik-Filme-Bücher/Büro-Schreibwaren' },
        { name: 'Comics', count: 58, route: '/Musik-Filme-Bücher/Comics' },
        { name: 'Fachbücher, Schule & Studium', count: 289, route: '/Musik-Filme-Bücher/Fachbücher-Schule-Studium' },
        { name: 'Film & DVD', count: 328, route: '/Musik-Filme-Bücher/Film-DVD' },
        { name: 'Musik & CDs', count: 375, route: '/Musik-Filme-Bücher/Musik-CDs' },
        { name: 'Musikinstrumente', count: 132, route: '/Musik-Filme-Bücher/Musikinstrumente' },
        { name: 'Weitere Musik, Filme & Bücher', count: 64, route: '/Musik-Filme-Bücher/Weitere-Musik-Filme-Bücher' }
    ];
        ...
        const fetchListings = async () => {
            try {
                ...
                if (response.ok) {
                    ...
                    setListings(data.filter(listing => listing.category === 'Musik, Filme & Bücher'));
                }
            } catch (error) {
            ...
            <div className="max-w-[1400px] mx-auto px-4 py-6">
                <div className="flex gap-6">
                    <aside className="w-96 flex-shrink-0 bg-white rounded-2xl shadow-lg p-6 h-fit sticky top-6">
                        ...
                        <div className="space-y-1">
                            {subcategories.map((sub) => (
                                <button key={sub.name} onClick={() => navigate(sub.route || '/Musik-Filme-Bücher')} className="w-full text-left px-4 py-3 rounded-lg transition-all flex items-center justify-between hover:bg-gray-50 text-gray-700 group">
                                    <span className="text-sm group-hover:text-purple-600 transition-colors">{sub.name}</span>
                    ...
                    <div className="flex-1">
                        <div className="bg-gradient-to-r from-purple-600 to-indigo-700 rounded-2xl shadow-xl p-8 mb-6 relative overflow-hidden">
                            ...
                            <div className="relative z-10">
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div><h1 className="text-4xl font-bold text-white mb-1">Musik, Filme & Bücher</h1><p className="text-white text-lg opacity-90">Alle Kategorien</p></div>
                                    </div>
                        ...
                        <div className="bg-white rounded-lg shadow-sm p-6">
                            ...
                            {loading ? <div className="flex justify-center items-center py-12"><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-purple-600"></div></div> : listings.length === 0 ? <div className="text-center py-12"><p className="text-gray-500 text-lg">Keine Anzeigen gefunden</p></div> : (
                                    ...
                                    {listings.map((listing) => (
                                        <div key={listing.id} className="bg-white rounded-xl shadow-md hover:shadow-xl transition-all duration-300 cursor-pointer group overflow-hidden flex" onClick={() => navigate(`/listing/${listing.id}`)}>
                                            ...
                                            <div className="flex-1 p-6 flex flex-col justify-between">
                                                <div><h3 className="font-semibold text-xl text-gray-900 mb-2 group-hover:text-purple-600 transition-colors">{listing.title}</h3><p className="text-gray-600 text-sm line-clamp-2 mb-3">{listing.description || 'Keine Beschreibung verfügbar'}</p></div>
                                                <div className="flex items-center justify-between">
                                                    <p className="text-2xl font-bold text-purple-600">{listing.price ? `${listing.price} €` : 'VB'}</p>
...
'''
    ...
    print("   ✅ Created MusikFilmeBuecherPage.js")
else:
    print("   ℹ️  Main page already exists")

print("\n🎉 Musik, Filme & Bücher category complete!")
