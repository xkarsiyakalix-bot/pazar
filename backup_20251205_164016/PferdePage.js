...
function PferdePage() {
    ...
    // New filter states
    const [selectedHorseType, setSelectedHorseType] = useState('');
    ...
    const filteredListings = listings.filter(listing =>
        ...
        (!selectedHorseType || listing.horseType === selectedHorseType) &&
    ...
    return (
        <div className="min-h-screen bg-gray-50"><div className="max-w-[1400px] mx-auto px-4 py-6"><div className="flex gap-6">
            <aside className="w-96 flex-shrink-0 bg-white rounded-2xl shadow-lg p-6 h-fit sticky top-6">                <div className="mb-6 pb-6 border-b border-gray-200">
                ...
                {/* Art (Horse Type) Filter */}
                <div className="mb-6">
                    <h4 className="font-bold text-gray-900 mb-3 text-base">Art</h4>
                    <div className="space-y-2">
                        {[
                            { value: 'Großpferde', count: 6338 },
                            { value: 'Kleinpferde & Ponys', count: 4148 }
                        ].map(horse => (
                            <label key={horse.value} className="flex items-center justify-between cursor-pointer hover:bg-gray-50 p-2 rounded-lg transition-colors">
                                <div className="flex items-center">
                                    <input type="radio" name="horseType" value={horse.value} checked={selectedHorseType === horse.value} onChange={(e) => setSelectedHorseType(e.target.value)} className="w-4 h-4 text-red-600 focus:ring-red-500" />
                                    <span className="ml-2 text-sm text-gray-700">{horse.value}</span>
                                </div>
                                <span className="text-xs text-gray-500">({horse.count.toLocaleString('de-DE')})</span>
                            </label>
                        ))}
                    </div>
                </div>
                ...
                <div className="mb-6">
                    ...
                    <div className="space-y-2">
                        {[
                            { value: 'Angebote', count: 10482 },
                            { value: 'Gesuche', count: 10 }
                        ].map(type => (
                ...
                <div className="mb-6">
                    ...
                    <div className="space-y-2">
                        {[
                            { value: 'Privat', count: 8971 },
                            { value: 'Gewerblich', count: 1521 }
                        ].map(provider => (
                ...
                <div className="mb-6">
                    ...
                    <div className="space-y-2 max-h-64 overflow-y-auto">
                        {[
                            { value: 'Baden-Württemberg', count: 763 },
                            { value: 'Bayern', count: 1307 },
                            { value: 'Berlin', count: 62 },
                            { value: 'Brandenburg', count: 492 },
                            { value: 'Bremen', count: 25 },
                            { value: 'Hamburg', count: 60 },
                            { value: 'Hessen', count: 592 },
                            { value: 'Mecklenburg-Vorpommern', count: 410 },
                            { value: 'Niedersachsen', count: 2256 },
                            { value: 'Nordrhein-Westfalen', count: 1834 },
                            { value: 'Rheinland-Pfalz', count: 631 },
                            { value: 'Saarland', count: 89 },
                            { value: 'Sachsen', count: 388 },
                            { value: 'Sachsen-Anhalt', count: 422 },
                            { value: 'Schleswig-Holstein', count: 816 },
                            { value: 'Thüringen', count: 345 }
                        ].map(state => (
                    ...
                    <div className="relative z-10"><div className="flex items-center justify-between"><div className="flex items-center gap-4">
                        <div className="w-16 h-16 bg-white bg-opacity-20 backdrop-blur-sm rounded-2xl flex items-center justify-center">
                            <svg className="w-10 h-10 text-white" viewBox="0 0 64 64" fill="none" stroke="currentColor">
                                <path d="M24 20 L28 16 L32 20 L32 28 L28 32 L24 28 Z" strokeWidth="2.5" fill="currentColor" opacity="0.3" />
                        ...
                        <div><h1 className="text-4xl font-bold text-white mb-1">Pferde</h1><p className="text-white text-lg opacity-90">Pferde & Zubehör</p></div>
                    </div>
