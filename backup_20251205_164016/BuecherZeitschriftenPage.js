...
function BuecherZeitschriftenPage() {
    ...
    const resetFilters = () => {
        setPriceFrom(''); setPriceTo(''); setSelectedArt(''); setVersand(''); setZustand('');
        setOfferType(''); setProviderType(''); setDirektKaufen(''); setPaketdienst(''); setSelectedState('');
    };

    const filteredListings = listings.filter(listing =>
        listing.category === 'Musik, Filme & Bücher' && listing.subCategory === 'Bücher & Zeitschriften' &&
        (!priceFrom || listing.price >= parseFloat(priceFrom)) &&
        (!priceTo || listing.price <= parseFloat(priceTo)) &&
        (!selectedArt || listing.art === selectedArt) &&
            ...
            <aside className="w-96 flex-shrink-0 bg-white rounded-2xl shadow-lg p-6 h-fit sticky top-6">
                ...
                <div className="mb-6">
                    ...
                    <div className="space-y-2">
                        {[
                            { value: 'Antiquarische Bücher', count: 130756 },
                            { value: 'Kinderbücher', count: 906250 },
                            { value: 'Krimis & Thriller', count: 170103 },
                            { value: 'Kunst & Kultur', count: 48098 },
                            { value: 'Sachbücher', count: 518302 },
                            { value: 'Science Fiction', count: 17400 },
                            { value: 'Unterhaltungsliteratur', count: 496952 },
                            { value: 'Zeitgenössische Literatur & Klassiker', count: 33158 },
                            { value: 'Zeitschriften', count: 83316 },
                            { value: 'Weitere Bücher & Zeitschriften', count: 166471 }
                        ].map(art => (
                            <label key={art.value} className="flex items-center justify-between cursor-pointer hover:bg-gray-50 p-2 rounded-lg transition-colors">
                                <div className="flex items-center">
                                    <input type="radio" name="art" value={art.value} checked={selectedArt === art.value} onChange={(e) => setSelectedArt(e.target.value)} className="w-4 h-4 text-red-600 focus:ring-red-500" />
                                    <span className="ml-2 text-sm text-gray-700">{art.value}</span>
                                </div>
                                <span className="text-xs text-gray-500">({art.count.toLocaleString('de-DE')})</span>
                            </label>
                        ))}
                    </div>
                </div>
                ...
                <div className="mb-6">
                    ...
                    <div className="space-y-2">
                        {[{ value: 'Versand möglich', count: 2300354 }, { value: 'Nur Abholung', count: 253107 }].map(v => (
                ...
                <div className="mb-6">
                    ...
                    <div className="space-y-2">
                        {[
                            { value: 'Neu', count: 267564 },
                            { value: 'Sehr Gut', count: 1228302 },
                            { value: 'Gut', count: 535311 },
                            { value: 'In Ordnung', count: 77337 }
                        ].map(z => (
                ...
                <div className="mb-6">
                    ...
                    <div className="space-y-2">
                        {[{ value: 'Angebote', count: 2566843 }, { value: 'Gesuche', count: 4225 }].map(type => (
                ...
                <div className="mb-6">
                    ...
                    <div className="space-y-2">
                        {[{ value: 'Privat', count: 2566385 }, { value: 'Gewerblich', count: 4683 }].map(provider => (
                ...
                <div className="mb-6">
                    ...
                    <div className="space-y-2">
                        <label className="flex items-center justify-between cursor-pointer hover:bg-gray-50 p-2 rounded-lg transition-colors">
                            ...
                            <span className="text-xs text-gray-500">(635.962)</span>
                        </label>
                    </div>
                </div>
                ...
                <div className="mb-6">
                    ...
                    <div className="space-y-2">
                        {[{ value: 'DHL', count: 1365940 }, { value: 'Hermes', count: 1363738 }].map(p => (
                ...
                <div className="mb-6">
                    ...
                    <div className="space-y-2">
                        {[
                            { value: 'Baden-Württemberg', count: 322118 }, { value: 'Bayern', count: 462174 }, { value: 'Berlin', count: 105998 },
                            { value: 'Brandenburg', count: 65327 }, { value: 'Bremen', count: 17384 }, { value: 'Hamburg', count: 71672 },
                            { value: 'Hessen', count: 197673 }, { value: 'Mecklenburg-Vorpommern', count: 33191 }, { value: 'Niedersachsen', count: 281785 },
                            { value: 'Nordrhein-Westfalen', count: 527010 }, { value: 'Rheinland-Pfalz', count: 126812 }, { value: 'Saarland', count: 27584 },
                            { value: 'Sachsen', count: 126358 }, { value: 'Sachsen-Anhalt', count: 45165 }, { value: 'Schleswig-Holstein', count: 115072 },
                            { value: 'Thüringen', count: 45745 }
                        ].map(state => (
                    ...
                    <div className="relative z-10"><div className="flex items-center justify-between"><div className="flex items-center gap-4">
                        <div className="w-16 h-16 bg-white bg-opacity-20 backdrop-blur-sm rounded-2xl flex items-center justify-center">
                            <svg className="w-10 h-10 text-white" viewBox="0 0 64 64" fill="none" stroke="currentColor">
                                <rect x="22" y="16" width="20" height="32" strokeWidth="2.5" fill="currentColor" opacity="0.3" rx="1" />
                                <rect x="22" y="16" width="20" height="32" strokeWidth="2.5" rx="1" />
                                <path d="M28 24 L36 24 M28 30 L36 30 M28 36 L36 36" strokeWidth="2" strokeLinecap="round" />
                            </svg>
                        </div>
                        <div><h1 className="text-4xl font-bold text-white mb-1">Bücher & Zeitschriften</h1><p className="text-white text-lg opacity-90">Bücher & Zeitschriften</p></div>
                    </div>
