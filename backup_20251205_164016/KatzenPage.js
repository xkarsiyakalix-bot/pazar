...
function KatzenPage() {
    ...
    const [vaccinatedChipped, setVaccinatedChipped] = useState('');
    const [officialPermission, setOfficialPermission] = useState('');
    ...
    const resetFilters = () => {
        ...
        setSelectedBreed('');
        setSelectedAge('');
        setVaccinatedChipped('');
        setOfficialPermission('');
        setOfferType('');
        setProviderType('');
        setSelectedState('');
    };
    ...
    const filteredListings = listings.filter(listing =>
        ...
        (!selectedBreed || listing.breed === selectedBreed) &&
        (!selectedAge || listing.age === selectedAge) &&
        (!vaccinatedChipped || listing.vaccinatedChipped === vaccinatedChipped) &&
        (!officialPermission || listing.officialPermission === officialPermission) &&
            ...
            <aside className="w-96 flex-shrink-0 bg-white rounded-2xl shadow-lg p-6 h-fit sticky top-6">                <div className="mb-6 pb-6 border-b border-gray-200">
                ...
                <div className="mb-6">
                    ...
                    <div className="space-y-2">
                        {[
                            { value: 'Britisch Kurzhaar', count: 603 },
                            { value: 'Hauskatze', count: 2020 },
                            { value: 'Maine Coon', count: 159 },
                            { value: 'Siam', count: 32 },
                            { value: 'Weitere Katzen', count: 526 }
                        ].map(breed => (
                            <label key={breed.value} className="flex items-center justify-between cursor-pointer hover:bg-gray-50 p-2 rounded-lg transition-colors">
                                <div className="flex items-center">
                                    <input type="radio" name="breed" value={breed.value} checked={selectedBreed === breed.value} onChange={(e) => setSelectedBreed(e.target.value)} className="w-4 h-4 text-red-600 focus:ring-red-500" />
                                    <span className="ml-2 text-sm text-gray-700">{breed.value}</span>
                                </div>
                                <span className="text-xs text-gray-500">({breed.count})</span>
                            </label>
                        ))}
                    </div>
                </div>
                ...
                {/* Alter (Age) Filter */}
                <div className="mb-6">
                    <h4 className="font-bold text-gray-900 mb-3 text-base">Alter</h4>
                    <div className="space-y-2">
                        {[
                            { value: 'jünger als 12 Monate', count: 386 },
                            { value: '12 Monate oder älter', count: 2952 }
                        ].map(age => (
                            <label key={age.value} className="flex items-center justify-between cursor-pointer hover:bg-gray-50 p-2 rounded-lg transition-colors">
                                <div className="flex items-center">
                                    <input type="radio" name="age" value={age.value} checked={selectedAge === age.value} onChange={(e) => setSelectedAge(e.target.value)} className="w-4 h-4 text-red-600 focus:ring-red-500" />
                                    <span className="ml-2 text-sm text-gray-700">{age.value}</span>
                                </div>
                ...
                {/* geimpft und gechipt Filter */}
                <div className="mb-6">
                    <h4 className="font-bold text-gray-900 mb-3 text-base">geimpft und gechipt</h4>
                    <div className="space-y-2">
                        {[
                            { value: 'Nein', count: 972 },
                            { value: 'Ja', count: 2366 }
                        ].map(option => (
                            <label key={option.value} className="flex items-center justify-between cursor-pointer hover:bg-gray-50 p-2 rounded-lg transition-colors">
                                <div className="flex items-center">
                                    <input type="radio" name="vaccinated" value={option.value} checked={vaccinatedChipped === option.value} onChange={(e) => setVaccinatedChipped(e.target.value)} className="w-4 h-4 text-red-600 focus:ring-red-500" />
                ...
                {/* Behördliche Erlaubnis Filter */}
                <div className="mb-6">
                    <h4 className="font-bold text-gray-900 mb-3 text-base">Behördliche Erlaubnis</h4>
                    <div className="space-y-2">
                        {[
                            { value: 'Nein', count: 1656 },
                            { value: 'Ja', count: 1680 }
                        ].map(option => (
                            <label key={option.value} className="flex items-center justify-between cursor-pointer hover:bg-gray-50 p-2 rounded-lg transition-colors">
                                <div className="flex items-center">
                                    <input type="radio" name="permission" value={option.value} checked={officialPermission === option.value} onChange={(e) => setOfficialPermission(e.target.value)} className="w-4 h-4 text-red-600 focus:ring-red-500" />
                                    <span className="ml-2 text-sm text-gray-700">{option.value}</span>
                                </div>
                                <span className="text-xs text-gray-500">({option.count})</span>
                            </label>
                ...
                <div className="mb-6">
                    ...
                    <div className="space-y-2">
                        {[
                            { value: 'Angebote', count: 3309 },
                            { value: 'Gesuche', count: 38 }
                        ].map(type => (
                ...
                <div className="mb-6">
                    ...
                    <div className="space-y-2">
                        {[
                            { value: 'Privat', count: 2481 },
                            { value: 'Gewerblich', count: 866 }
                        ].map(provider => (
                ...
                <div className="mb-6">
                    ...
                    <div className="space-y-2 max-h-64 overflow-y-auto">
                        {[
                            { value: 'Baden-Württemberg', count: 329 },
                            { value: 'Bayern', count: 479 },
                            { value: 'Berlin', count: 161 },
                            { value: 'Brandenburg', count: 212 },
                            { value: 'Bremen', count: 35 },
                            { value: 'Hamburg', count: 85 },
                            { value: 'Hessen', count: 225 },
                            { value: 'Mecklenburg-Vorpommern', count: 108 },
                            { value: 'Niedersachsen', count: 398 },
                            { value: 'Nordrhein-Westfalen', count: 730 },
                            { value: 'Rheinland-Pfalz', count: 165 },
                            { value: 'Saarland', count: 21 },
                            { value: 'Sachsen', count: 111 },
                            { value: 'Sachsen-Anhalt', count: 97 },
                            { value: 'Schleswig-Holstein', count: 109 },
                            { value: 'Thüringen', count: 82 }
                        ].map(state => (
                    ...
                    <div className="relative z-10"><div className="flex items-center justify-between"><div className="flex items-center gap-4">
                        <div className="w-16 h-16 bg-white bg-opacity-20 backdrop-blur-sm rounded-2xl flex items-center justify-center">
                            <svg className="w-10 h-10 text-white" viewBox="0 0 64 64" fill="none" stroke="currentColor">
                                <circle cx="32" cy="30" r="8" strokeWidth="2.5" fill="currentColor" opacity="0.3" />
                                <circle cx="32" cy="30" r="8" strokeWidth="2.5" strokeLinecap="round" />
                                <path d="M26 24 L22 18 M38 24 L42 18" strokeWidth="2.5" strokeLinecap="round" />
                                <path d="M28 32 L30 34 L34 34 L36 32" strokeWidth="2" strokeLinecap="round" />
                            </svg>
                        </div>
                        <div><h1 className="text-4xl font-bold text-white mb-1">Katzen</h1><p className="text-white text-lg opacity-90">Katzen & Kitten</p></div>
                    </div>
