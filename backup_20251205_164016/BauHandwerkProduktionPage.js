...
function BauHandwerkProduktionPage() {
    ...
    const resetFilters = () => {
        setCategory(''); setHourlyWageFrom(''); setHourlyWageTo(''); setSelectedJobType(''); setWorkingTime('');
        setOfferType(''); setProviderType(''); setSelectedState('');
    };

    const filteredListings = listings.filter(listing =>
        listing.category === 'Jobs' && listing.subCategory === 'Bau, Handwerk & Produktion' &&
        ...
        (!selectedJobType || listing.jobType === selectedJobType) && (!workingTime || listing.workingTime === workingTime) &&
    ...
    return (
        <div className="min-h-screen bg-gray-50"><div className="max-w-[1400px] mx-auto px-4 py-6"><div className="flex gap-6">
            <aside className="w-96 flex-shrink-0 bg-white rounded-2xl shadow-lg p-6 h-fit sticky top-6 overflow-y-auto" style={{ maxHeight: 'calc(100vh - 48px)' }}>
                <div className="mb-6 pb-6 border-b border-gray-200">
                    ...
                    <div className="space-y-2 mt-3">
                        ...
                        <button className="text-left px-3 py-2 rounded-lg text-sm transition-all bg-blue-600 text-white shadow-md flex items-center justify-between ml-8" style={{ width: 'calc(100% - 2rem)' }}>
                            <span>Bau, Handwerk & Produktion</span>
                            <button onClick={(e) => { e.stopPropagation(); navigate('/Jobs'); }} className="text-white hover:text-blue-200 transition-colors" title="Kategorie schließen">
                ...
                <div className="flex items-center justify-between mb-6"><h3 className="font-bold text-gray-900 text-lg">Filter</h3><button onClick={resetFilters} className="text-sm text-blue-600 hover:text-blue-700 font-medium">Zurücksetzen</button></div>
                ...
                <div className="mb-6"><h4 className="font-bold text-gray-900 mb-3 text-base">Stundenlohn</h4><div className="grid grid-cols-2 gap-3">
                    <div><label className="block text-xs text-gray-600 mb-1">Von</label><input type="number" value={hourlyWageFrom} onChange={(e) => setHourlyWageFrom(e.target.value)} placeholder="0" className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-300" /></div>
                    <div><label className="block text-xs text-gray-600 mb-1">Bis</label><input type="number" value={hourlyWageTo} onChange={(e) => setHourlyWageTo(e.target.value)} placeholder="∞" className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-300" /></div>
                </div></div>
                ...
                <div className="mb-6">
                    ...
                    <div className="space-y-2 max-h-64 overflow-y-auto">
                        {[
                            { value: 'Bauhelfer/-in', count: 1785 },
                            { value: 'Dachdecker/-in', count: 938 },
                            { value: 'Elektriker/-in', count: 12108 },
                            { value: 'Fliesenleger/-in', count: 712 },
                            { value: 'Maler/-in', count: 2689 },
                            { value: 'Maurer/-in', count: 1114 },
                            { value: 'Produktionshelfer/-in', count: 13166 },
                            { value: 'Schlosser/-in', count: 4475 },
                            { value: 'Tischler/-in', count: 1831 },
                            { value: 'Weitere Berufe', count: 48795 }
                        ].map(type => (
                            <label key={type.value} className="flex items-center justify-between cursor-pointer hover:bg-gray-50 p-2 rounded-lg transition-colors">
                                <div className="flex items-center">
                                    <input type="radio" name="jobType" value={type.value} checked={selectedJobType === type.value} onChange={(e) => setSelectedJobType(e.target.value)} className="w-4 h-4 text-blue-600 focus:ring-blue-500" />
                ...
                <div className="mb-6">
                    ...
                    <div className="space-y-2">
                        {[{ value: 'Vollzeit', count: 47145 }, { value: 'Teilzeit', count: 2190 }].map(time => (
                            <label key={time.value} className="flex items-center justify-between cursor-pointer hover:bg-gray-50 p-2 rounded-lg transition-colors">
                                <div className="flex items-center">
                                    <input type="radio" name="workingTime" value={time.value} checked={workingTime === time.value} onChange={(e) => setWorkingTime(e.target.value)} className="w-4 h-4 text-blue-600 focus:ring-blue-500" />
                ...
                <div className="mb-6">
                    ...
                    <div className="space-y-2">
                        {[{ value: 'Angebote', count: 78847 }, { value: 'Gesuche', count: 8786 }].map(type => (
                            <label key={type.value} className="flex items-center justify-between cursor-pointer hover:bg-gray-50 p-2 rounded-lg transition-colors">
                                <div className="flex items-center">
                                    <input type="radio" name="offerType" value={type.value} checked={offerType === type.value} onChange={(e) => setOfferType(e.target.value)} className="w-4 h-4 text-blue-600 focus:ring-blue-500" />
                                    <span className="ml-2 text-sm text-gray-700">{type.value}</span>
                                </div>
                                <span className="text-xs text-gray-500">({type.count.toLocaleString('de-DE')})</span>
                            </label>
                ...
                <div className="mb-6">
                    ...
                    <div className="space-y-2">
                        {[{ value: 'Privat', count: 8767 }, { value: 'Gewerblich', count: 78866 }].map(provider => (
                            <label key={provider.value} className="flex items-center justify-between cursor-pointer hover:bg-gray-50 p-2 rounded-lg transition-colors">
                                <div className="flex items-center">
                                    <input type="radio" name="provider" value={provider.value} checked={providerType === provider.value} onChange={(e) => setProviderType(e.target.value)} className="w-4 h-4 text-blue-600 focus:ring-blue-500" />
                ...
                <div className="mb-6">
                    ...
                    <div className="space-y-2 max-h-64 overflow-y-auto">
                        {[
                            { value: 'Baden-Württemberg', count: 7804 }, { value: 'Bayern', count: 12119 }, { value: 'Berlin', count: 4710 },
                            { value: 'Brandenburg', count: 2432 }, { value: 'Bremen', count: 1144 }, { value: 'Hamburg', count: 2362 },
                            { value: 'Hessen', count: 4675 }, { value: 'Mecklenburg-Vorpommern', count: 1371 }, { value: 'Niedersachsen', count: 13681 },
                            { value: 'Nordrhein-Westfalen', count: 21361 }, { value: 'Rheinland-Pfalz', count: 3283 }, { value: 'Saarland', count: 570 },
                            { value: 'Sachsen', count: 3746 }, { value: 'Sachsen-Anhalt', count: 2638 }, { value: 'Schleswig-Holstein', count: 3369 },
                            { value: 'Thüringen', count: 2368 }
                        ].map(state => (
                            <label key={state.value} className="flex items-center justify-between cursor-pointer hover:bg-gray-50 p-2 rounded-lg transition-colors">
                                <div className="flex items-center">
                                    <input type="radio" name="state" value={state.value} checked={selectedState === state.value} onChange={(e) => setSelectedState(e.target.value)} className="w-4 h-4 text-blue-600 focus:ring-blue-500" />
            ...
            <div className="flex-1">
                <div className="bg-blue-600 rounded-2xl shadow-xl p-8 mb-6 relative overflow-hidden"><div className="absolute inset-0 opacity-10"><div className="absolute top-0 left-0 w-full h-full" style={{ backgroundImage: 'repeating-linear-gradient(45deg, transparent, transparent 35px, rgba(255,255,255,.1) 35px, rgba(255,255,255,.1) 70px)' }}></div></div>
                    <div className="relative z-10"><div className="flex items-center justify-between"><div className="flex items-center gap-4">
                        <div className="w-16 h-16 bg-white bg-opacity-20 backdrop-blur-sm rounded-2xl flex items-center justify-center">
                            <svg className="w-10 h-10 text-white" viewBox="0 0 64 64" fill="none" stroke="currentColor">
                                <path d="M24 38 L24 28 L32 22 L40 28 L40 38" strokeWidth="2.5" fill="currentColor" opacity="0.3" />
                                <path d="M24 38 L24 28 L32 22 L40 28 L40 38" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
                                <rect x="28" y="32" width="8" height="6" strokeWidth="2" fill="currentColor" opacity="0.5" />
                                <path d="M20 38 L44 38" strokeWidth="2.5" strokeLinecap="round" />
                            </svg>
                        </div>
                        <div><h1 className="text-4xl font-bold text-white mb-1">Bau, Handwerk & Produktion</h1><p className="text-white text-lg opacity-90">Bau, Handwerk & Produktion</p></div>
                    </div>
                ...
                <div className="bg-white rounded-lg shadow-sm p-6"><h2 className="text-2xl font-bold text-gray-900 mb-4">{filteredListings.length} Anzeigen</h2>
                    {loading ? (<div className="flex justify-center items-center py-12"><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600"></div></div>) : filteredListings.length === 0 ? (<div className="text-center py-12"><p className="text-gray-500 text-lg">Keine Anzeigen gefunden</p></div>) : (
                        <div className="space-y-4">{filteredListings.map((listing) => (
                            <div key={listing.id} className="bg-white rounded-xl shadow-md hover:shadow-xl transition-all duration-300 cursor-pointer group overflow-hidden flex" onClick={() => navigate(`/product/${listing.id}`)}><div className="relative w-64 h-48 flex-shrink-0 bg-gray-100"><img src={listing.image || listing.images?.[0] || 'https://via.placeholder.com/300x200'} alt={listing.title} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-300" />
                                <button onClick={(e) => { e.stopPropagation(); toggleFavorite(listing.id); }} className="absolute top-3 right-3 bg-white rounded-full p-2 shadow-lg hover:scale-110 transition-transform">
                                    <svg className={`w-5 h-5 ${favorites.includes(listing.id) ? 'fill-blue-500 text-blue-500' : 'text-gray-400'}`} fill={favorites.includes(listing.id) ? 'currentColor' : 'none'} stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" /></svg>
                                </button>
                            </div>
                                <div className="flex-1 p-6 flex flex-col justify-between">
                                    <div>
                                        <h3 className="font-semibold text-xl text-gray-900 mb-2 group-hover:text-blue-600 transition-colors">{listing.title}</h3>
                                        <p className="text-gray-600 text-sm line-clamp-2 mb-3">{listing.description}</p>
                                    </div>
                                    <div className="flex items-center justify-between">
                                        <p className="text-2xl font-bold text-blue-600">{typeof listing.price === 'string' ? listing.price : listing.price ? `${listing.price.toLocaleString('de-DE')} €` : 'VB'}</p>
