...
function HandyTelefonPage() {
    const navigate = useNavigate();
    const [selectedArt, setSelectedArt] = useState([]);
    const [selectedGeraetZubehoer, setSelectedGeraetZubehoer] = useState([]);
    ...
    const [verkaeufergarantie, setVerkaeufergarantie] = useState(false);
    ...
    const filteredListings = listings.filter(listing =>
        ...
        listing.subCategory === 'Handy & Telefon' &&
        (selectedArt.length === 0 || selectedArt.includes(listing.art)) &&
        (selectedGeraetZubehoer.length === 0 || selectedGeraetZubehoer.includes(listing.geraetZubehoer)) &&
        ...
        (!verkaeufergarantie || listing.verkaeufergarantie) &&
        ...
        <div className="min-h-screen bg-gray-50"><div className="max-w-[1400px] mx-auto px-4 py-6"><div className="flex gap-6">
            <aside className="w-96 flex-shrink-0 bg-white rounded-2xl shadow-lg p-6 h-fit sticky top-6">
                ...
                <div className="mb-6 pb-6 border-b border-gray-200">
                    <h3 className="font-bold text-gray-900 mb-3 text-base">Elektronik</h3>
                    <div className="space-y-2 mt-3">
                        <button
                            onClick={() => navigate('/Elektronik')}
                        ...
                        <button
                            className="w-full text-left px-3 py-2 rounded-lg text-sm transition-all bg-red-600 text-white shadow-md flex items-center justify-between"
                        >
                            <span>Handy & Telefon</span>
                            <button
                                onClick={(e) => {
                                    e.stopPropagation();
                                    navigate('/Elektronik');
                ...
                </div>                <div className="mb-6 pb-6 border-b border-gray-200">
                ...
                <div className="flex items-center justify-between mb-6"><h3 className="font-bold text-gray-900 text-lg">Filter</h3><button onClick={() => {
                    setSelectedArt([]);
                    setSelectedGeraetZubehoer([]);
                    setSelectedVersand([]);
                    setSelectedFarbe([]);
                    setSelectedZustand([]);
                    setPriceFrom('');
                    setPriceTo('');
                    setVerkaeufergarantie(false);
                ...
                <div className="mb-6 pb-6 border-b border-gray-200">
                    ...
                    <div className="space-y-2 max-h-64 overflow-y-auto">
                        {[
                            { value: 'Apple', label: 'Apple', count: '230.028' },
                            { value: 'Google', label: 'Google', count: '1.616' },
                            { value: 'HTC', label: 'HTC', count: '800' },
                            { value: 'Huawei', label: 'Huawei', count: '2.212' },
                            { value: 'LG', label: 'LG', count: '1.061' },
                            { value: 'Motorola', label: 'Motorola', count: '3.493' },
                            { value: 'Nokia', label: 'Nokia', count: '10.683' },
                            { value: 'Samsung', label: 'Samsung', count: '91.750' },
                            { value: 'Siemens', label: 'Siemens', count: '7.539' },
                            { value: 'Sony', label: 'Sony', count: '4.188' },
                            { value: 'Xiaomi', label: 'Xiaomi', count: '3.323' },
                            { value: 'Faxgeräte', label: 'Faxgeräte', count: '836' },
                            { value: 'Telefone', label: 'Telefone', count: '23.869' },
                            { value: 'Weitere Handys & Telefone', label: 'Weitere Handys & Telefone', count: '89.507' }
                        ].map(option => (
                ...
                {/* Gerät & Zubehör Filter */}
                <div className="mb-6 pb-6 border-b border-gray-200">
                    <h4 className="font-bold text-gray-900 mb-3 text-base">Gerät & Zubehör</h4>
                    <div className="space-y-2">
                        {[
                            { value: 'Gerät', label: 'Gerät', count: '158.478' },
                            { value: 'Zubehör', label: 'Zubehör', count: '160.860' },
                            { value: 'Gerät & Zubehör', label: 'Gerät & Zubehör', count: '150.696' }
                        ].map(option => (
                            <label key={option.value} className="flex items-center justify-between cursor-pointer group">
                                <div className="flex items-center">
                                    <input type="checkbox" checked={selectedGeraetZubehoer.includes(option.value)} onChange={() => toggleFilter(option.value, selectedGeraetZubehoer, setSelectedGeraetZubehoer)} className="w-4 h-4 text-red-600 border-gray-300 rounded focus:ring-red-500" />
                                    <span className="ml-2 text-sm text-gray-700 group-hover:text-gray-900">{option.label}</span>
                                </div>
                                <span className="text-xs text-gray-500">({option.count})</span>
                            </label>
                ...
                <div className="mb-6 pb-6 border-b border-gray-200">
                    ...
                    <div className="space-y-2">
                        {[
                            { value: 'Versand möglich', label: 'Versand möglich', count: '380.704' },
                            { value: 'Nur Abholung', label: 'Nur Abholung', count: '87.686' }
                        ].map(option => (
                            <label key={option.value} className="flex items-center justify-between cursor-pointer group">
                                <div className="flex items-center">
                ...
                <div className="mb-6 pb-6 border-b border-gray-200">
                    ...
                    <div className="space-y-2 max-h-64 overflow-y-auto">
                        {[
                            { value: 'Silber', label: 'Silber', count: '25.859' },
                            { value: 'Weiß', label: 'Weiß', count: '45.991' },
                            { value: 'Blau', label: 'Blau', count: '32.728' },
                            { value: 'Grau', label: 'Grau', count: '23.809' },
                            { value: 'Gelb', label: 'Gelb', count: '1.479' },
                            { value: 'Braun', label: 'Braun', count: '3.180' },
                            { value: 'Schwarz', label: 'Schwarz', count: '147.245' },
                            { value: 'Gold', label: 'Gold', count: '10.339' },
                            { value: 'Roségold', label: 'Roségold', count: '7.083' },
                            { value: 'Orange', label: 'Orange', count: '2.294' },
                            { value: 'Pink', label: 'Pink', count: '6.030' },
                            { value: 'Rot', label: 'Rot', count: '6.562' },
                            { value: 'Grün', label: 'Grün', count: '10.510' },
                            { value: 'Lila', label: 'Lila', count: '8.119' },
                            { value: 'Beige', label: 'Beige', count: '2.774' },
                            { value: 'Andere Farben', label: 'Andere Farben', count: '56.142' }
                        ].map(option => (
                            <label key={option.value} className="flex items-center justify-between cursor-pointer group">
                                <div className="flex items-center">
                                    <input type="checkbox" checked={selectedFarbe.includes(option.value)} onChange={() => toggleFilter(option.value, selectedFarbe, setSelectedFarbe)} className="w-4 h-4 text-red-600 border-gray-300 rounded focus:ring-red-500" />
                                    <span className="ml-2 text-sm text-gray-700 group-hover:text-gray-900">{option.label}</span>
                                </div>
                                <span className="text-xs text-gray-500">({option.count})</span>
                            </label>
                ...
                <div className="mb-6 pb-6 border-b border-gray-200">
                    ...
                    <div className="space-y-2">
                        {[
                            { value: 'Neu', label: 'Neu', count: '123.424' },
                            { value: 'Sehr Gut', label: 'Sehr Gut', count: '197.934' },
                            { value: 'Gut', label: 'Gut', count: '83.217' },
                            { value: 'In Ordnung', label: 'In Ordnung', count: '21.444' },
                            { value: 'Defekt', label: 'Defekt', count: '9.435' }
                        ].map(option => (
                            <label key={option.value} className="flex items-center justify-between cursor-pointer group">
                                <div className="flex items-center">
                ...
                {/* Verkaufslabels Filter */}
                <div className="mb-6 pb-6 border-b border-gray-200">
                    <h4 className="font-bold text-gray-900 mb-3 text-base">Verkaufslabels</h4>
                    <label className="flex items-center cursor-pointer group">
                        <input type="checkbox" checked={verkaeufergarantie} onChange={(e) => setVerkaeufergarantie(e.target.checked)} className="w-4 h-4 text-red-600 border-gray-300 rounded focus:ring-red-500" />
                        <span className="ml-2 text-sm text-gray-700 group-hover:text-gray-900">Verkäufergarantie</span>
                    </label>
                </div>
                ...
                <div className="mb-6 pb-6 border-b border-gray-200">
                    ...
                    <div className="space-y-2">
                        {[
                            { value: 'Angebote', label: 'Angebote', count: '466.910' },
                            { value: 'Gesuche', label: 'Gesuche', count: '4.017' }
                        ].map(option => (
                            <label key={option.value} className="flex items-center justify-between cursor-pointer group">
                                <div className="flex items-center">
                ...
                <div className="mb-6 pb-6 border-b border-gray-200">
                    ...
                    <div className="space-y-2">
                        {[
                            { value: 'Privat', label: 'Privat', count: '452.137' },
                            { value: 'Gewerblich', label: 'Gewerblich', count: '18.790' }
                        ].map(option => (
                            <label key={option.value} className="flex items-center justify-between cursor-pointer group">
                                <div className="flex items-center">
                ...
                <div className="mb-6 pb-6 border-b border-gray-200">
                    ...
                    <label className="flex items-center justify-between cursor-pointer group">
                        ...
                        <span className="text-xs text-gray-500">(137.893)</span>
                    </label>
                </div>
                ...
                <div className="mb-6 pb-6 border-b border-gray-200">
                    ...
                    <div className="space-y-2">
                        {[
                            { value: 'DHL', label: 'DHL', count: '298.858' },
                            { value: 'Hermes', label: 'Hermes', count: '276.408' }
                        ].map(option => (
                            <label key={option.value} className="flex items-center justify-between cursor-pointer group">
                                <div className="flex items-center">
                ...
                <div className="mb-6">
                    ...
                    <div className="space-y-2 max-h-64 overflow-y-auto">
                        {[
                            { value: 'Baden-Württemberg', label: 'Baden-Württemberg', count: '56.854' },
                            { value: 'Bayern', label: 'Bayern', count: '75.505' },
                            { value: 'Berlin', label: 'Berlin', count: '25.329' },
                            { value: 'Brandenburg', label: 'Brandenburg', count: '11.188' },
                            { value: 'Bremen', label: 'Bremen', count: '4.195' },
                            { value: 'Hamburg', label: 'Hamburg', count: '12.083' },
                            { value: 'Hessen', label: 'Hessen', count: '39.144' },
                            { value: 'Mecklenburg-Vorpommern', label: 'Mecklenburg-Vorpommern', count: '7.010' },
                            { value: 'Niedersachsen', label: 'Niedersachsen', count: '48.376' },
                            { value: 'Nordrhein-Westfalen', label: 'Nordrhein-Westfalen', count: '106.412' },
                            { value: 'Rheinland-Pfalz', label: 'Rheinland-Pfalz', count: '21.718' },
                            { value: 'Saarland', label: 'Saarland', count: '5.844' },
                            { value: 'Sachsen', label: 'Sachsen', count: '20.727' },
                            { value: 'Sachsen-Anhalt', label: 'Sachsen-Anhalt', count: '9.574' },
                            { value: 'Schleswig-Holstein', label: 'Schleswig-Holstein', count: '17.050' },
                            { value: 'Thüringen', label: 'Thüringen', count: '9.918' }
                        ].map(option => (
                            <label key={option.value} className="flex items-center justify-between cursor-pointer group">
                                <div className="flex items-center">
                    ...
                    <div className="relative z-10"><div className="flex items-center justify-between"><div className="flex items-center gap-4">
                        <div className="w-16 h-16 bg-white bg-opacity-20 backdrop-blur-sm rounded-2xl flex items-center justify-center">
                            <svg className="w-10 h-10 text-white" viewBox="0 0 64 64" fill="none" stroke="currentColor">
                                <rect x="24" y="16" width="16" height="32" strokeWidth="2.5" fill="currentColor" opacity="0.3" rx="2" />
                                <rect x="24" y="16" width="16" height="32" strokeWidth="2.5" strokeLinecap="round" rx="2" />
                                <circle cx="32" cy="43" r="2" fill="currentColor" />
                            </svg>
                        </div>
                        <div><h1 className="text-4xl font-bold text-white mb-1">Handy & Telefon</h1><p className="text-white text-lg opacity-90">Handys & Telefone</p></div>
                    </div>
