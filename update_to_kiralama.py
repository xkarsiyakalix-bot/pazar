...
print("="*80)
print("ÖDÜNÇ VERME → KIRALAMA GÜNCELLEMESİ")
print("="*80)

# Update the subcategory from "Ödünç Verme" to "Kiralama"
print("\n1. Güncelleme yapılıyor...")
print("-"*80)

result = supabase.table('listings').update({
    'sub_category': 'Kiralama'
}).eq('sub_category', 'Ödünç Verme').eq('category', 'Ücretsiz & Takas').execute()
