    ...
    const navItems = [
        {
            id: 'home', path: '/', label: 'Ana Sayfa', icon: (active) => (
                <svg className={`w-6 h-6 ${active ? 'text-red-600' : 'text-gray-500 dark:text-neutral-400'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        },
        {
            id: 'add', path: '/add-listing', label: 'İlan Ver', isSpecial: true, icon: (active) => (
                <div className="bg-red-600 rounded-full p-2 -mt-6 shadow-lg border-4 border-white dark:border-neutral-900 transition-transform active:scale-95">
                        ...
                        >
                            <div className="relative">
                                ...
                                {item.id === 'messages' && unreadCount > 0 && (
                                    <span className="absolute -top-1.5 -right-2 bg-red-600 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full min-w-[18px] text-center border-2 border-white dark:border-neutral-900 shadow-sm">
                                        {unreadCount}
                                    </span>
                                )}
                            </div>
                            {!item.isSpecial && (
                                <span className={`text-[10px] mt-1 font-medium truncate w-full px-1 text-center ${isActive ? 'text-red-600' : 'text-gray-500 dark:text-neutral-400'}`}>
                                    {item.label}
                                </span>
                            )}
                        </button>
